    ...
    public class BankService : IBankService
    {
        ...
        public async Task<IQueryable<Bank>> GetBankByParams(BankSearchDto bankSearchDto)
        {
            ...
            if (!string.IsNullOrWhiteSpace(bankSearchDto.Name))
                query = query.Where(x => x.BankName.Contains(bankSearchDto.Name) && (x.Status == bankSearchDto.status)).Include(b=>b.Address);

            if (!string.IsNullOrWhiteSpace(bankSearchDto.IFSC))
                query = query.Where(x => x.IFSCCode.Contains(bankSearchDto.IFSC) && (x.Status == bankSearchDto.status)).Include(b => b.Address);

            if (!string.IsNullOrWhiteSpace(bankSearchDto.BranchName))
                query = query.Where(x => x.BranchName.Contains(bankSearchDto.BranchName) && (x.Status == bankSearchDto.status)).Include(b => b.Address);
